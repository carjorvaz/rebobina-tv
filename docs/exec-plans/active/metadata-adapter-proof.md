...
- The UI remains unchanged when swapping fixture data for normalized provider
  ...
  `rebobina.catalog_source=provider-snapshot`, the smoke harness
  `--catalog-source provider-snapshot` option, and the named
  `provider-movie-watch` flow for the RTP2 detail-derived movie; the launcher
  default remains the safe local fixture.
